...
def get_or_create_collection():
    # ChromaDB uses its own lightweight default embeddings
    return chroma_client.get_or_create_collection("persona")
...
def ingest_resume(pdf_path: str = "resume.pdf"):
    if os.path.exists(pdf_path):
        ...
        from persona import YOUR_BACKGROUND
        full_text = YOUR_BACKGROUND
    ...
    print(f"Ingested {len(chunks)} resume chunks")
...
def ingest_commits(github_username: str):
    ...
    for repo in repos:
        ...
        print(f"Ingested commits for {repo_name}")
